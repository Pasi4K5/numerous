...
namespace Numerous.Web.DependencyInjection;

public static class DiHelper
{
    public static IEnumerable<(ServiceType serviceType, Type type, Type impl)> GetServices()
    {
        return Assembly.GetEntryAssembly()?.GetReferencedAssemblies().SelectMany(assemblyName =>
                   Assembly.Load(assemblyName).GetTypes().Where(t =>
